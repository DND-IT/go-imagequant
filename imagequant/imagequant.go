...
func (q *QImg) Run() (image.Image, error) {
	...
	if q.MaxQuality != DefaultMaxQuality || q.MinQuality != DefaultMinQuality {
		...
		if liqError != C.LIQ_OK {
			C.liq_attr_destroy(handle)
			C.free(cRaw8BitPixels)
			return nil, fmt.Errorf("c call to liq_set_quality() failed with code %v", liqError)
		}
	}
	...
	liqError = C.liq_write_remapped_image(liqResult, inputImage, cRaw8BitPixels, C.ulong(pixelSize))
	if liqError != C.LIQ_OK {
		C.liq_result_destroy(liqResult)
		C.liq_image_destroy(inputImage)
		C.liq_attr_destroy(handle)
		C.free(cRaw8BitPixels)
		return nil, fmt.Errorf("c call to liq_write_remapped_image() failed with code %v", liqError)
	}
